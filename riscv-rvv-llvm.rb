class RiscvRvvLlvm < Formula
    ...
    def install
        ...
        # Build compiler-rt
        system "cd .. && mkdir build_compiler-rt && cd build_compiler-rt && cmake -G 'Unix Makefiles' ../compiler-rt -DCMAKE_INSTALL_PREFIX=#{prefix}/$LLVM_RESOURCEDIR/ -DCMAKE_TRY_COMPILE_TARGET_TYPE=STATIC_LIBRARY -DCMAKE_CROSSCOMPILING=True -DCMAKE_SYSTEM_NAME=Linux -DCMAKE_BUILD_TYPE=Release -DCOMPILER_RT_BUILD_BUILTINS=ON -DCOMPILER_RT_BUILD_SANITIZERS=OFF -DCOMPILER_RT_BUILD_XRAY=OFF -DCOMPILER_RT_BUILD_LIBFUZZER=OFF -DCOMPILER_RT_BUILD_PROFILE=OFF -DCOMPILER_RT_BUILD_MEMPROF=OFF -DCOMPILER_RT_BUILD_XRAY_NO_PREINIT=OFF -DCOMPILER_RT_SANITIZERS_TO_BUILD=none -DCMAKE_C_COMPILER=$CC -DCMAKE_CXX_COMPILER=$CXX -DCMAKE_AR=$AR -DCMAKE_NM=$NM -DCMAKE_RANLIB=$RANLIB -DLLVM_CONFIG_PATH=$LLVM_CONFIG -DCMAKE_C_COMPILER_TARGET=$TARGET64 -DCOMPILER_RT_DEFAULT_TARGET_ONLY=ON -DCMAKE_C_FLAGS='${TARGET_CFLAGS}' -DCMAKE_CXX_FLAGS='${TARGET_CXXFLAGS}' -DCMAKE_EXE_LINKER_FLAGS='${TARGET_LDFLAGS}' -DCOMPILER_RT_BAREMETAL_BUILD=ON -DCOMPILER_RT_OS_DIR='' && make -j10 && make install"
      test do
        system "false"
      end
    end
